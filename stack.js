...
function stack(){
  ...
  var touchyStart = function() {
    touchStartVal = d3.event.changedTouches[0].clientY;
  };

  var touchyEnd = function() {
    var delta;
    touchEndVal = d3.event.changedTouches[0].clientY;
    if (touchStartVal > touchEndVal) {
      delta = 1;
    } else if (touchStartVal < touchEndVal) {
      delta = -1;
    }
  ...
  d3.select(window)
      ...
      .on("touchstart", touchyStart)
      // .on("touchmove", touchy)
      .on("touchend", touchyEnd)
      .each(resize);
